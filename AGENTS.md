...
# aicli
_2026-04-29 13:47 UTC | Memory synced: 2026-04-29_
...
- All backend LLM prompts stored in YAML under backend/memory/prompts/ with roles (pm.yaml, architect.yaml, developer.yaml, reviewer.yaml, react_base.yaml); delivery_type classification uses project tech_stack context
...
- UI transparency badges: _waitingBadge() showing '⏳ X days waiting' (grey ≤3d, amber 4–7d, red >7d) for pending items; _openDaysBadge() showing '📂 X days open' for approved use cases
- Delivery type routing: work items classified with delivery_type (code_change/feature/bug/refactor/doc/infrastructure) using Haiku classifier; maps to appropriate pipeline based on tech_stack and file coupling context
