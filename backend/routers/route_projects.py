...
async def _synthesize_with_llm(
    ...
    try:
        ...
        _prompt_tpl = None
        _prompt_file = Path(__file__).parent.parent / "memory" / "yaml_config" / "command_memory.yaml"
        if _prompt_file.exists():
            try:
                import yaml as _yaml
                entries = _yaml.safe_load(_prompt_file.read_text()) or []
                if isinstance(entries, list):
                    entry = next((e for e in entries if e.get("key") == "project_synthesis"), None)
                    _prompt_tpl = (entry or {}).get("system", "")
            except Exception:
                pass
        if not _prompt_tpl:
...
async def generate_memory(project_name: str):
    ...
    try:
        ...
        # Embed code.md for MCP search_facts
        _mf_obj._embed_code_md(project_name)
