...
# aicli
_2026-04-29 13:45 UTC | Memory synced: 2026-04-29_
...
- **memory_files**: YAML under backend/memory/prompts/; /memory POST endpoint single source of truth for project_state.json
...
- All backend LLM prompts stored in YAML under backend/memory/prompts/ with roles (pm.yaml, architect.yaml, developer.yaml, reviewer.yaml, react_base.yaml); delivery_type classification uses project tech_stack context to improve pipeline routing accuracy
...
- Database optimization: batch queries replace N+1 patterns; single WHERE name = ANY(%s) per category for hotspot/coupling checks; token counting: len(text) // 4; _update_item_tags uses executemany instead of N execute calls
...
- Delivery type routing: work items classified with delivery_type (code_change/feature/bug/refactor/doc/infrastructure) using Haiku classifier with full project tech stack and file coupling context; maps to appropriate pipeline
