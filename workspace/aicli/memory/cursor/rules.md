...
> Managed by aicli. Run `/memory` to refresh. Generated: 2026-05-02 22:42 UTC
...
- [2026-05-02] All pipeline suppose to be based on yaml config. Also the one that user is doing (are shaped as yaml file). Also I do se
- [2026-05-02] I did not finish the prompt. will write that again -  All pipeline suppose to be based on yaml config. Also the one that
- [2026-05-02] Ok. So now even not approved are embeded. what happend if item change - is it reembed the data for consistency ? Also I 
- [2026-05-02] I would like to add some ui improvemtn - when execution start a new fix window is opend . is it possible to resize the w
- [2026-05-02] Looks better. there is not point to have all logs under exection log , and also the same logs under each step (architect