...
# aicli
_2026-04-29 18:01 UTC | Memory synced: 2026-04-29_
...
- YAML ↔ DB sync: role and pipeline YAML files seed database on backend startup; UI edits save to database only; conflicting sync strategy: user chooses between UI-first (DB overwrites YAML) or YAML-first (YAML overwrites DB) on each backend restart
...
- `backend/routers/route_agent_roles.py` — score 6.0 (4 commits, 958 lines)
