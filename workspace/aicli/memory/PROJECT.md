...
- MCP Catalog wiring verification: confirmed all 10 MCPs fully implemented with activate/deactivate/edit modals, role-usage chips, and dedicated view under main left nav Workflows section
- Role UI styling: colored pill backgrounds for Provider+Model matching Settings design; full background shapes for model display; status pills (BASED/UPDATED/EXTERNAL) with green/orange/amber colors; removed redundant small inline restore buttons
- System prompts cleanup: soft-deleted 28 legacy system roles; retained only 3 canonical presets (Coding—General, Design & Planning, Review & Quality); all 10 roles correctly mapped to canonical presets; reset-to-defaults fully functional
- Project filesystem cleanup: removed dead cli/ folder, aicli/documents/, aicli/features/; aligned workspace/aicli/pipelines/ with workspace/_templates/pipelines/; consolidated all pipeline YAML configs with role_* and pl_* prefixes
- Yaml config organization: separated memory-related YAMLs (command_memory.yaml, feature_detect.yaml, memory.yaml) into backend/memory/yaml_config/; agent/pipeline prompts (agent_react.yaml, event_generation.yaml) into backend/agents/yaml_config/
- Pipeline and execution next steps: ready to focus on pipeline management UI (create/edit pipelines, configure stages, test execution flow) with retry logic moving from roles to pipeline-level YAML configuration
...
- System prompts consolidation: 3 shared system prompt presets in workspace/_templates/pipelines/system_prompts.yaml (coding_general, design_and_planning, review_and_quality); mng_agent_roles.system_prompt_preset references presets by ID; new roles default to canonical preset on creation
- Tool and MCP management: tools grouped by category (files, git, bash, etc.) with multi-select category dropdowns per role in UI; MCPs stored in mng_mcp_servers table; MCP Catalog accessible from main left nav under Workflows section with activate/deactivate/edit cards
- Role base snapshot feature: POST /agent-roles/{id}/set-base snapshots current system_prompt, provider, model, tools, mcp as base_snapshot; Restore button resets role to last saved base; visual status pills (BASED/UPDATED/EXTERNAL) indicate state relative to base
