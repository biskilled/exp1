...
- Execute bar unified input styling: output folder, docs dropdown, files multi-select aligned in single row with consistent height/padding; dropdown styled with explicit background, shadow, z-index to float above prompts
- Chat history by session: CLI sessions now visible in Chat tab (fixed source column mapping: hook strips source from tags, stores in src column); session grouping restored; execution logs below in history panel
- Role library parity with pipelines: direct role execution with same exec bar (output folder, doc search, file upload, textarea); history logs display below; supports document reference + file upload simultaneously
- Bug fixes: Remove unused mem_ai_events columns, remove lifecycle tags from Planner, fix drag-and-drop in Planner, fix tagging UI error, fix commit-prompt reference display, fix PROJECT.md file loading timeout, fix recent projects list, fix backend startup race condition, fix undefined column errors in route_history/route_entities
- Pipeline node configuration: max_iterations mandatory per node, stateless/continue-on-fail/approval-gate checkboxes aligned and highlighted in execution bar, checkpoint control UI positioned directly below node diagram
