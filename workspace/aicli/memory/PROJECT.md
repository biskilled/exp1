...
- Pipeline mode filtering: Added mode_use_case and mode_item checkboxes to Settings → Workflows pipelines panel; only marked pipelines appear in Use Cases run menu and item run menus respectively
- Execute bar refinement: doc picker redesigned as proper dropdown widget matching other selects; file upload button using onclick trigger; output folder, docs, and files controls aligned in single row with consistent heights
- Pipeline execution logs: History panel below run diagram showing per-role execution output and results persistence
...
- Chat history visibility: Backend SQL reads src column for message sourcing; CLI chat sessions grouped by session in History view with prompts/responses properly displayed
- ToolUseBlock attribute errors in agent execution: Fixed 'ToolUseBlock' object has no attribute 'get' in agents/agent.py line 596 when processing tool responses from Claude
...
- Role YAML as factory defaults: workspace/_templates/pipelines/roles/*.yaml are read-only templates seeded with ON CONFLICT DO NOTHING on backend startup; mng_agent_roles DB is single source of truth at runtime; UI edits persist in DB only; base_snapshot JSONB stores pristine state for versioning and reset
- System prompts: 3 canonical presets (Coding—General, Design & Planning, Review & Quality) in workspace/_templates/pipelines/system_prompts.yaml; all roles default to one preset; soft-deleted all legacy system roles
...
- Pipeline & role activation: Settings → Workflows dual-pane shows all roles/pipelines with activation checkboxes; only activated items appear in main tabs and are executable; pipeline activation requires all constituent roles to be activated
...
- Execute bar unified input: output folder combobox (default = pipeline/role name) + searchable project docs dropdown + multi-file upload in same row; files shown as removable chips above textarea; supports multiple document and file selections; integrated into pipelines, roles, and use cases
...
- Pipeline execution entry points: (1) Pipelines tab (◈) with node diagram and exec bar, (2) /pipeline [name] slash command in Chat with prior session prompts as input, (3) /role [name] in Chat, (4) Use Cases section with approval gating and mode filter (use_case/item modes), (5) Role Library card direct execution
