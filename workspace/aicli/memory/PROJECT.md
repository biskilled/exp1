...
- Pipeline execution framework: GET /agent-roles/pipelines/{name} returns full pipeline config with stage role details and default_temperature; POST /agents/pipeline-runs starts async DAG execution with role overrides (provider/model/temperature); UI execution panel displays stage outputs with async coordination
- Role activation in Workflows Settings: Settings/Workflow renamed to Roles & Pipelines with dual-pane dual-column view; left pane shows all roles/pipelines with activation checkboxes; only activated items appear in role/pipeline tabs; reset-to-base and role-versioning via mng_agent_role_versions
- UI visual design consolidation: role pills styled with provider color backgrounds (matching Settings Agent Role design); status badges (BASED green, UPDATED orange, EXTERNAL amber); all buttons and model displays have background fills for clarity
- MCP Catalog in main nav: dedicated MCP Catalog view under Workflows group; card-based UI shows all 10 MCPs with activate/deactivate/edit modals; role-usage chips display which roles reference each MCP; multi-select dropdown in role editor for MCP assignment
- Tool category bundles: tool selection now by category (git/files/memory) instead of individual items; categories show tool count and names; tools organized under role editor with category-based multi-select dropdown
- Work item UI bug fixes: nested item rendering for doc_type/bug/feature categories; category label display in rows; tag visibility with proper width; commit/prompt counting fixed; phase metadata and session filtering in History tab
...
- Role YAML as factory defaults: workspace/_templates/pipelines/roles/role_*.yaml are read-only templates; ON CONFLICT DO NOTHING seeds only new roles on startup; mng_agent_roles DB is single source of truth at runtime; UI edits persist in DB only; base_snapshot JSONB stores pristine state; POST /agent-roles/{id}/refresh reloads YAML and updates DB
