...
- MCP tool restoration complete: verify route_entities.py deletion impact fully resolved in all routes
- Backend prompt consolidation: verify all LLM prompts use renamed YAML under backend/memory/prompts/; event_commit.yaml vs command_memory.yaml naming clarified
...
- Code.md generation completeness: verify all public symbols from commits included; confirm file coupling and hotspot tables populated; test is_latest BOOLEAN index and post-commit refresh
- PROJECT.md file loading timeout: investigation of >60 second load time; likely caused by large commit history or inefficient query in memory endpoint
- Project facts deprecation: mem_ai_project_facts table effectively dead; no automatic population path; verify cleanup or repurposing decision
- Planner tags removal: transition from planner_tags to mem_work_items hierarchy complete; verify no orphaned references remain
- Tag counter updating in Planner UI: counter display not updating when tags added/removed; requires frontend state sync
- Unused mem_ai_events columns audit: cleanup deferred columns (language, file_path, etc.) from schema
