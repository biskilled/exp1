...
- Fix PROJECT.md file loading timeout (>60s) — PROJECT.md now read once in single pass but performance audit ongoing; may need database indices on project, wi_type, user_status
- Fix undefined column errors in route_entities (line 359: t.lifecycle) and route_history (line 228: event_type) — these columns were removed in migration m080 but route code not yet updated
...
- Remove lifecycle tags from Planner UI (11 active bugs remain: drag-and-drop, category display, archive toggles, tagging UI errors, [object object] tag display)
...
- MCP server: 14 tools (search_memory, get_project_state, list_work_items, get_work_item, list_commits, search_commits, etc.) dispatched via REST endpoints in agents/mcp/server.py with unified dispatch; stdio transport running locally on developer machine with no auth
...
- All backend LLM prompts stored in YAML under backend/memory/prompts/ with roles (pm.yaml, architect.yaml, developer.yaml, reviewer.yaml, react_base.yaml); project facts stored in mem_ai_project_facts table with conflict detection and embedding after /memory POST synthesis
...
- Delivery type routing: work items classified with delivery_type (code_change/feature/bug/refactor/doc/infrastructure) using Haiku classifier with project tech stack context; maps to appropriate pipeline (developer, architect, infrastructure, etc.)
