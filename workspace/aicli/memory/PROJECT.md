...
- MCP tool restoration: verify route_entities.py deletion impact; ensure all work item tools (create_entity, sync_github_issues, list_work_items) route to correct endpoints
...
- Route history DB fixes: resolve 'event_type' column not exists errors; fix JSONB type casting in commit sync operations
- Drag-and-drop validation: confirm both Work Items and Use Cases support merge/parent-child linking with same implementation (elementFromPoint detection)
