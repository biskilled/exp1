...
- Pipeline execution testing & debugging: fixing ToolUseBlock attribute errors (getattr fallback), messages.N tool_result sequencing (Anthropic format), cost calculation for max_iterations path, and stage timeout issues (Architect stage taking 122K tokens / 103s)
- Execution history & logging UI: stage detail panels showing input/output snapshots, steps_json ReAct trace visualization, verdict banner with score_dots (●/○), session-level chat history grouped by source (CLI vs web)
- Stage visibility and rich output: rendering input_snapshot as text/file summary and output_snapshot artifacts (code changes, file outputs), collapsible ReAct steps panel per stage, duration/cost per-stage breakdown
- In-place DOM updates optimization: avoiding full page reloads on role/pipeline activation toggles and mode flag changes; reducing layout jank and improving perceived responsiveness
- MCP server HTTP migration: converting from stdio transport to HTTP REST for multi-user support (currently 10-tool stdio server)
- React refactor planning: evaluating React + Vite + Electron as frontend replacement for vanilla JS; assessing bundle size, DX, and ecosystem fit
