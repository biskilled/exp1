...
- Execute bar unified input: output folder combobox (default = pipeline name) + searchable project docs dropdown + multi-file upload in same row; files shown as removable chips above textarea; dropdown styled with explicit background, shadow, and z-index
- Bug fixes: Remove unused mem_ai_events columns, remove lifecycle tags from Planner, fix drag-and-drop in Planner, fix tagging UI error, fix commit-prompt reference display, fix PROJECT.md file loading timeout, fix recent projects list missing aiCli project, fix backend startup race condition, fix undefined column errors in route_history/route_entities
- Checkpoint control UI: Max retry, stateless, continue-on-fail, and approval gate checkboxes aligned and highlighted in execution bar; exec bar positioned directly below node diagram with proper layout flow
- Role library & pipeline direct execution parity: Roles and pipelines both support unified exec bar (output folder, doc search, file upload, multi-file chips, prompt textarea); execution logs displayed in History panel below; support for document reference and file upload simultaneously
...
- Execute bar unified input: output folder combobox (default = pipeline/role name) + searchable project docs dropdown + multi-file upload in same row; files shown as removable chips above textarea; supports multiple selections; integrated into both Pipelines tab (DAG builder) and Roles library direct execution
