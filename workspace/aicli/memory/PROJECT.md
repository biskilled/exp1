...
- Pipeline/role execution UI: exec bar positioning fixed (moved inside gw-main, not inside canvas-area), doc picker redesigned as proper dropdown widget matching multi-select style, files shown as chips above textarea, output folder combobox with default name
- Chat history and session grouping: Backend SQL reads src column for message sourcing, CLI chat sessions now visible in History grouped by session with prompts/responses properly displayed, session order and phase filtering working
- Use Cases pipeline execution: Run button now shows dropdown of available pipelines, fixed dropdown positioning to not be clipped by card overflow, execution screen renders properly alongside use case details
- Dashboard restoration: Mirror Data cards (commits, prompts, items, messages—total + 24h), Work Items/Use Cases section (approved vs waiting-to-approve, total embeddings, removed scoring per item), Pipeline Runs health tiles (recent runs table, cost breakdown)
- Bug fixes: ToolUseBlock.get() error in agent.py fixed, unused mem_ai_events columns removal, lifecycle tags removed from Planner UI, tag display and counting in work items fixed, commit-prompt reference rendering
- PROJECT.md file loading optimization: addressing >60s timeouts during project initialization
...
- Role YAML as factory defaults: workspace/_templates/pipelines/roles/*.yaml are read-only templates seeded with ON CONFLICT DO NOTHING on backend startup; mng_agent_roles DB is single source of truth at runtime; base_snapshot JSONB stores pristine state for versioning and restore-to-base
- System prompts: 3 shared canonical presets (Coding—General, Design & Planning, Review & Quality) in workspace/_templates/pipelines/system_prompts.yaml; system_roles table contains only 3 canonical entries; soft-delete old presets on reset
...
- Pipeline execution: 4-agent async DAG triggered on approved items under approved use cases; executed via asyncio.gather; max_iterations mandatory per node; per-node checkboxes: max_retry, stateless, continue-on-fail, approval-gate
...
- Execute bar unified input: output folder combobox (default = pipeline/role name) + searchable project docs dropdown + multi-file upload in same row; files shown as removable chips above textarea; supports multiple document and file selections
...
- Pipeline execution entry points: (1) Pipelines tab with node diagram and exec bar, (2) /pipeline [name] slash command in Chat, (3) /role [name] in Chat, (4) Use Cases section with approval gating; execution runs async DAG on approved items only
