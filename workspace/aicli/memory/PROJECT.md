...
- Code quality audit (sessions 37–40): verified all modules <1500 lines, SQL queries optimized (no N+1), recursive CTEs bounded depth<20, no dead code; token counting switched to len(text)//4; _load_context() split into _query_db_into_ctx() and _parse_project_md().
- Performance investigation: PROJECT.md load times >60s traced to get_project_context() aggregation inefficiency; refactored with separate DB query and markdown parsing paths to reduce round-trips.
- Memory file convergence: unified get_project_context() path for both /memory POST and commit/work-item triggers; removed duplicate render paths (render_root_claude_md) so all files use same DB source.
- Work item auto-closure on commits: regex matcher ('fixes BU0012', 'closes FE0001') auto-sets score_status=5, score_importance=5; requires user approval to mark user_status='done'.
- Date cascade validation: child work items cannot have due_date later than parent use_case due_date; enforced in _apply_date_rules() on reparenting.
