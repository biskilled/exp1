---
description: Run AgentDesk backend commands — memory, status, pipeline, search, commit, entities, agent testing
---

# AgentDesk Command Bridge

Route the user's command to the AgentDesk backend at `http://localhost:8000`.
The active project is `agentdesk` unless specified otherwise.
...
| Command | Action |
|---------|--------|
| `/memory` | `curl -s -X POST http://localhost:8000/projects/agentdesk/memory` — generates and syncs all 5 memory files (CLAUDE.md, MEMORY.md, context.md, rules.md, copilot.md). Show which files were generated and any suggested tags. |
| `/status` | `curl -s http://localhost:8000/projects/agentdesk` — show project metadata, then also call MCP tool `get_project_state` for facts + active work items. Summarize concisely. |
...
| `/push [message]` | `curl -s -X POST "http://localhost:8000/git/agentdesk/commit-push" -H "Content-Type: application/json" -d '{"commit_message":"<message or empty string>"}'` — stage all changes, generate LLM commit message, commit and push. Show commit hash + message. |
| `/pull` | `curl -s -X POST "http://localhost:8000/git/agentdesk/pull"` — git pull latest from remote. Show result. |
| `/git-status` | `curl -s "http://localhost:8000/git/agentdesk/status"` — show staged/unstaged files and current branch. |
| `/embed [commits\|prompts]` | Without arg: both. `commits` → `POST /memory/agentdesk/embed-commits`. `prompts` → `POST /memory/agentdesk/embed-prompts`. Show events_created. |
...
1. POST to `http://localhost:8000/agents/run` with body:
   ```json
   {"role": "<role_name>", "task": "<task>", "project": "agentdesk"}
   ```
...
1. POST to `http://localhost:8000/agents/run-pipeline` with body:
   ```json
   {"pipeline": "<pipeline_name>", "task": "<task>", "project": "agentdesk"}
   ```
...
- All MCP tools are from the `agentdesk_project` MCP server (already configured).
