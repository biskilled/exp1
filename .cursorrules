...
- Repeated chore commits after claude cli sessions (14 commits 2026-04-27 to 2026-04-28) — investigating whether auto_commit_push.sh is firing correctly or if git state is being duplicated.
- MCP server fully operational: all 10+ tools dispatched correctly via REST endpoints; work item classification and embedding pipeline functional; ready for Claude Code sessions to use shared memory.
- Work item review queue: approved use cases driving 4-agent DAG pipeline; commit-sourced items auto-closure via regex patterns awaiting user approval.
- Hotspot recency weighting: 180-day half-life formula integrated into code.md generation and memory queries to deprioritize stale files.
