<!-- Last updated: 2026-04-29 19:36 UTC -->
## Project: aicli
...
- Role YAML → DB sync: workspace/_templates/pipelines/roles/*.yaml are read-only factory defaults; ON CONFLICT DO NOTHING seeds only new roles on startup; mng_agent_roles DB table is single source of truth for runtime; UI edits persist in DB across restarts
...
- Fix commit sync batch upsert and database schema migration persistence: old tables still appearing after migration; verifying DB state matches latest migration (m080+)
...
- System prompts UI consolidation: duplicate role entries and UI refresh issues after reset-defaults endpoint; consolidation from 28 → 3 presets in progress

_Last updated: 2026-04-29 19:36 UTC_